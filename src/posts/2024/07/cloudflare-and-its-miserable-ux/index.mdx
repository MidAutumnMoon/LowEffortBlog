...
All deployments created using wrangler from Github Action **did not** go production. More confusingly, running the same wrangler deploy command on local machine still didn't update the production.

What?

There wasn't any hidden CLI options and related environment variable made no effects, Google yielded nothing, Cloudflare forum had no similar reports and no issues on this problem were found. Hours passed YAML poked, it was getting more frustrating and out loud calling me to quite.
