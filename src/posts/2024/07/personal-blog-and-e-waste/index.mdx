...
JavaScript is very poorly designed language, and the JavaScript dependency ecosystem is straight up garbage in terms of manageability and security. Developers praise for "velocity", praise for the unrestricted ability to pull in dependencies, but the tax of having this freedom isn't noted anywhere.

The feeling of clean code is good, and the feeling of having clear overview of dataflow in project is the great pleasure. JavaScript with its dependency offers none of it. Ah, [*Motivation is Dead*](https://www.youtube.com/watch?v=lCBF_XSFmng).
...
Some nice minimalistic text based blogs for reference:
...
Ever visited another person's blog on your phone while taking subway, but the page doesn't follow your finger but instead maneuvers left and right? This almost always caused by element overflow, like embedding a video without capping the max width.
...
My rule of thumb is, articles should only use one level of heading, and use deeper one only if truly needed. Readers will lost track of sections pretty quickly if headings are nested too deep. Another way is to simply not use headings at all.
