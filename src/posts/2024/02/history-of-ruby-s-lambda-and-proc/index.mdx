...
To my surprise, `lambda` is actually the elder brother of `proc` and both of them dated back to 1995. However *proc* the name became such a signature of Ruby.

Stepping into the next era, things will get complicated very quick. But first take a quick break.


# Break Time: Life of `do end` Syntax

`do end` is an iconic syntax of Ruby. But it was not born this way or stayed the same through the history of Ruby. It with `Proc` is the unsplittable pair of Ruby foundation, so this section summarizes its evolution to this day (2024).

It seems that Ruby had `do using end` syntax very early on, and may even be the first of few keywords of Ruby. But it was changed to brace style `{param| ...body}` in `v0.51`, Oct 14 1994, meaning there were no `do end` syntax for a period of time.

With the release of `v0.76`, May 19 1995, two vertical bars were used to quote the parameters of block `{ |param| ...body }`, which is one of the forms of today's block syntax.

Advancing time, during the development of `v0.99`, Aug 29 1996, the `do end` syntax was finally added back. Here's the translated changelog[^1996_do_end_changelog]:
...
Well, that's *The History of Ruby's `do end` Block and `{ || }` Block*. Nice title for an article isn't it?
