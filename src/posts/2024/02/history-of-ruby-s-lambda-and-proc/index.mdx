...
At the [beginning of ~~time~~ git history](https://github.com/ruby/ruby/tree/v0_49), Ruby already had iterator and `do end` of its early form. Under the hood, both `for` loops and `do end` are implemented using a special data structure called `BLOCK`, which contains code, can accept arguments and has its own environment[^0_49_eval_c], making it essentially `closure` in modern terms.

[^0_49_eval_c]: https://github.com/ruby/ruby/blob/v0_49/eval.c#L36-L41

Here's a relic sample[^relic_sample] found in `v0.49` from 1994, the `do end` block looks so strange yet so familiar.
